...
def main(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch == 1:
            A_se_wave.data = torch.where(torch.rand_like(A_se_wave.data) < 0.5,
                                         torch.tensor(0, device=A_se_wave.device, dtype=torch.float), A_se_wave.data)
