...
class TemporalConvNet(nn.Module):   #dropout_prob dilation_step 2-3
    def __init__(self, num_inputs, num_channels, kernel_sizes=[2, 2, 2], dropout_prob=0.1, dilation_step=1):
        super(TemporalConvNet, self).__init__()
        self.dilation_step = dilation_step
        layers = []
        num_levels = len(num_channels)

        # 确保kernel_sizes列表的长度与卷积层的数量相匹配
        if len(kernel_sizes) != num_levels:
            raise ValueError("Length of kernel_sizes must match the number of levels")

        for i in range(num_levels):
            dilation_size = self.calculate_dilation(i)
            in_channels = num_inputs if i == 0 else num_channels[i - 1]
            out_channels = num_channels[i]
            kernel_size = kernel_sizes[i]
            padding = (kernel_size - 1) * dilation_size

            conv = nn.Conv2d(in_channels, out_channels, (1, kernel_size), dilation=(1, dilation_size), padding=(0, padding))
    ...
    def calculate_dilation(self, level):
        return 1 + self.dilation_step * level
    ## 适度增加膨胀大小
#return 1 + self.dilation_step * level * 1.5

# 更快增加膨胀大小
#return 1 + self.dilation_step * level * 2

# 更慢增加膨胀大小
#return 1 + self.dilation_step * level * 0.5
...
class STGCNBlock(nn.Module):
    def __init__(self, in_channels, out_channels, num_nodes, A_hat):
        ...
        self.odeg = ODEG(out_channels[-1], 12, A_hat, time=6, max_num_layers=5)  #4--5
